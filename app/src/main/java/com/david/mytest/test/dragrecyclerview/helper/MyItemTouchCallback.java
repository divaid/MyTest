...
//        super(ItemTouchHelper.RIGHT|ItemTouchHelper.UP|ItemTouchHelper.DOWN|ItemTouchHelper.LEFT, ItemTouchHelper.RIGHT);
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        if (recyclerView.getLayoutManager() instanceof GridLayoutManager) {
            ...
            final int swipeFlags = 0;//只有类似于listView时才可滑动删除
            return makeMovementFlags(dragFlags, swipeFlags);
        } else {
            final int dragFlags = ItemTouchHelper.UP | ItemTouchHelper.DOWN;
            final int swipeFlags = ItemTouchHelper.RIGHT;//向右滑动删除
            return makeMovementFlags(dragFlags, swipeFlags);
        }
    }
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            //侧滑删除时改变Item的透明度，也可实现自定义的侧滑删除（具体参考http://www.jianshu.com/p/0c1984bc9383）
