...
import com.david.mytest.data.TestCheckAllAdapter;
...
/**
 * Created by david on 2016/8/29.
 * <p/>
 * listView实现全选,全不选功能
 */
public class AllCheckActivity extends BaseActivity implements AdapterView.OnItemClickListener {
    private static final String TAG = "AllCheckActivity";
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.bt_confirm:
                ...
                List<com.david.mytest.requestBean.TestCheckBean> allChecked = new ArrayList<>();
                for (int i = 0; i < checkedMap.size(); i++) {

                    if (checkedMap.get(i) == null) {    //防止出现空指针,如果为空,证明没有被选中
                        continue;
                    }else if (checkedMap.get(i)){
                        com.david.mytest.requestBean.TestCheckBean testCheckBean = mData.get(i);
                        allChecked.add(testCheckBean);
                    }
                }
                for (com.david.mytest.requestBean.TestCheckBean bean : allChecked) {
